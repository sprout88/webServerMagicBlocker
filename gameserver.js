 ...
 io.sockets.on('connection', function (socket) {
     ...
     socket.on('signUp', function (userData) {
        isValidNewCredential(userData).then(function (res) {
            if (res){
                console.log("사용자가 입력한 비밀번호 암호화...")
                userData.salted_password = crypto.pbkdf2Sync(userData.password, salt, 1, 32, 'sha512').toString('base64');
                userData.salt = salt;
...
function isValidNewCredential(userData) {
 ...
 function isCorrectCredential(userData) { //유저가 입력한 비밀번호. 틀릴수도있음.
    //console.log(crypto.pbkdf2Sync(userData.password, userData.salt, 1, 32, 'sha512').toString('base64'));
    return new Promise(function (callback) {
        ...
        dbo.collection(MONGO_REPO).find(query).toArray(function (err, result) {
            if (err) throw err;
            //&&userData.password==crypto.pbkdf2Sync(result[0].password, result[0].salt, 1, 32, 'sha512').toString('base64')
            if (result.length != 0) {
                //crypto.pbkdf2Sync(result[0].password, result[0].salt, 1, 32, 'sha512').toString('base64')
                if(result[0].salted_password==crypto.pbkdf2Sync(userData.password, result[0].salt, 1, 32, 'sha512').toString('base64')){
...
function insertCredential(data) {
    var account = {
        ...
        salt: data.salt,
        salted_password:data.salted_password,
        points: 0,
    };
